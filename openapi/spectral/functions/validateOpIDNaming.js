    ...
    switch (method) {
      case "delete":
        if (!DELETE.includes(suffix)) {
          return [
            {
              message: `${method.toUpperCase()} ${path} - ${operationId}: first segment after the namespace should start with one of: ${DELETE.join(", ")}. Prefer 'delete'. Example OperationID: droplet_delete`,
            },
          ];
        }
        break;
